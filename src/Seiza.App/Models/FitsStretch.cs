        ...
        get
        {
            ...
            switch (ModelType)
            {
                case FitsBackgroundModelType.Automatic:
                    if (!double.IsFinite(Ridge) || Ridge < 0)
                    {
                ...
                default:
                    throw new InvalidOperationException($"Unsupported background model '{ModelType}'.");
            }
        }
    }
...
internal sealed class FitsImageProcessingConfiguration : IEquatable<FitsImageProcessingConfiguration>
{
    public FitsImageProcessingConfiguration(
        ...
        : this(
            stretchStack,
            extractsBackground ? FitsBackgroundConfiguration.CreateLegacyDefault() : null,
            deconvolution,
            interactivePreview)
    {
    }
    ...
    public bool ExtractsBackground => BackgroundConfiguration is not null;
